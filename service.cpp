...
void ServiceImpl::initializeStorage(){
  if (!storageclient_.has("G_chirpid")) {
    storageclient_.put("G_chirpid", "1");
    std::cout << "[LOG] inserted G_chirpid " << storageclient_.get("G_chirpid")  << std::endl;
  } else {
    std::cout << "didn't insert G_chirpid" << std::endl;
  }

  if (!storageclient_.has("G_userid")) {
    storageclient_.put("G_userid", "1");
    std::cout << "[LOG] inserted G_userid " << storageclient_.get("G_userid") << std::endl;
  } else {
